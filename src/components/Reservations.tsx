...
const Reservations = () => {
  ...
  const [stats, setStats] = useState([
    { label: "Total Reservations", value: "0", color: "bg-gradient-to-br from-blue-500 to-blue-600", icon: TrendingUp, change: "+12%" },
            ...
            {stats.map((stat, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${stat.change.startsWith('+') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                    }`}>
                    {stat.change}
                  </span>
                </div>
          ...
          <div className="bg-white rounded-2xl max-w-5xl w-full shadow-2xl my-8">
            ...
            <div className="px-8 py-4 border-b border-gray-200 flex gap-6">
              {['General', 'Chats', 'Activity log', 'Changes'].map((tab) => (
                <button key={tab} className={`text-sm font-medium pb-2 border-b-2 transition-colors ${tab === 'General'
            ...
            <div className="p-6 space-y-2">
              {(
                ...
                <button
                  ...
                  disabled={formSubmitting}
                  className={`w-full px-4 py-3 rounded-xl text-sm font-medium text-left capitalize transition-all flex items-center gap-3 ${statusChangeBooking.status === status
                      ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-sm"
                      : "bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 hover:shadow-sm"
                    }`}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${statusChangeBooking.status === status ? 'bg-white/20' : 'bg-white'
                    }`}>
              ...
              {(["pending", "partial", "paid", "refunded"] as const).map(
                (payment) => (
                  <button
                    ...
                    disabled={formSubmitting}
                    className={`w-full px-4 py-3 rounded-xl text-sm font-medium text-left capitalize transition-all flex items-center gap-3 ${paymentChangeBooking.payment === payment
                        ? "bg-gradient-to-r from-green-500 to-green-600 text-white shadow-sm"
                        : "bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 hover:shadow-sm"
                      }`}
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${paymentChangeBooking.payment === payment ? 'bg-white/20' : 'bg-white'
                      }`}>
